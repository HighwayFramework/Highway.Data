...
namespace Highway.Data.Security.DataEntitlements.Expressions
{
    ...
    public class IncludeSecurityExpressionBuilder
    {
        ...
        /// </summary>
        /// <typeparam name="T">The <see cref="System.Type"/> to be secured.</typeparam>
        ...
        /// <returns></returns>
        public Expression<Func<T, bool>> GenerateSecurityPredicate<T>(IEntitlementProvider entitlements, SecuredMappingsCache mappingCache)
        {
            ...
            var leafPropertySecuredRelationships = mappingCache.GetSecuredRelationships(leafProperty.DeclaringType);
            ...
            var securedRelationship = leafPropertySecuredRelationships.SingleOrDefault(x => x.SecuredBy == leafProperty.PropertySingularType);
            if (!RelationshipNeedsToBeSecured<T>(entitlements, securedRelationship))
            {
                return null;
            }

            var entitledIds = entitlements.GetEntitledIds(securedRelationship.SecuredBy);
            var filterLambda = securedRelationship.GetSimplePredicate(entitledIds);
        ...
        private LambdaExpression GetLambdaForEnumerableProperty<T>(IncludePathMetadataFragment includePathMetadataFragment, LambdaExpression filterLambda)
        {
            ...
            return SecuredExtensions.CombineCollectionPropertySelectorWithPredicate(propertyLambda, filterLambda);
        }
        ...
        private bool RelationshipNeedsToBeSecured<T>(IEntitlementProvider entitlements, SecuredRelationship securedRelationship)
        {

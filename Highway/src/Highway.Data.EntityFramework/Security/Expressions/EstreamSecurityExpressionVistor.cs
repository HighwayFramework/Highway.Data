...
namespace Highway.Data.EntityFramework.Security.Expressions
{
    ...
    /// </summary>
    public class EstreamSecurityExpressionVistor : ExpressionVisitor
    {
        private readonly List<IncludeSecurityExpressionBuilder> _metadatas;

        /// <summary>
        ///     Initializes a new instance of the <see cref="EstreamSecurityExpressionVistor" /> class.
        /// </summary>
        public EstreamSecurityExpressionVistor()
        {
        ...
        /// <summary>
        ///     Visits the children of the <see cref="T:System.Linq.Expressions.MethodCallExpression" /> and collects metadata
        ///     about secured included properties.
        /// </summary>
        /// <param name="node">
        ...
        private void AddNestedPropertyDetails(IncludeSecurityExpressionBuilder includeMetadata,
            string propertyPathSegment, IncludePathMetadataFragment previousIncludePathMetadataFragment)
        {
            var currentProperty =
                previousIncludePathMetadataFragment.PropertySingularType.GetProperty(propertyPathSegment);
            var parameterExpression = Expression.Parameter(previousIncludePathMetadataFragment.PropertySingularType);
            var propertyAccessMetadata =
                new IncludePathMetadataFragment(previousIncludePathMetadataFragment.PropertySingularType,
                    currentProperty.PropertyType, Expression.MakeMemberAccess(parameterExpression, currentProperty));
            includeMetadata.IncludedPropertyDetails.Add(propertyAccessMetadata);
        }

        private void AddPropertyDetails(IncludeSecurityExpressionBuilder includeMetadata, Type typeUnderQuery,
            ParameterExpression parameterForTypeUnderQuery, string propertyPathSegment)
        {
        ...
        private void AddRootPropertyDetails(IncludeSecurityExpressionBuilder includeMetadata, Type typeUnderQuery,
            ParameterExpression parameterForTypeUnderQuery, string propertyPathSegment)
        {
            var currentProperty = typeUnderQuery.GetProperty(propertyPathSegment);
            var propertyAccessMetadata = new IncludePathMetadataFragment(typeUnderQuery, currentProperty.PropertyType,
                Expression.MakeMemberAccess(parameterForTypeUnderQuery, currentProperty));
            includeMetadata.IncludedPropertyDetails.Add(propertyAccessMetadata);
        }
        ...
        private void CollectSecuredIncludeMetaDatas(MethodCallExpression callNode)
        {
            ...
            var typeUnderQuery = Extensions.TypeExtensions.ToSingleType(queryType);
            var parameterForTypeUnderQuery = Expression.Parameter(typeUnderQuery);

            var includedPropertyPathSegments = includedPropertyPath.Split('.');
                // Example:  {"Entity", "Property", "Property"}
